...
# 設定資料庫
# 自動修正 PostgreSQL 連線字串 (適配 Zeabur)
...
class PrepItem(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    category = db.Column(db.String(50), nullable=False) # e.g., 'doc' (證件), 'wear' (衣物), 'water' (水上), 'other' (其他)
    name = db.Column(db.String(100), nullable=False)
...
def seed_data():
    ...
    preps = [
        ...
        PrepItem(category='doc', name='現金 (小島部分店家只收現)'),
        PrepItem(category='doc', name='船票訂位證明 / 民宿地址'),
        PrepItem(category='water', name='泳衣 / 泳褲 (建議兩套替換)'),
        PrepItem(category='water', name='蛙鏡 / 呼吸管 (若有個人衛生考量)'),
        PrepItem(category='water', name='浴巾 / 毛巾 (玩水用)'),
        PrepItem(category='water', name='環保防曬乳 (友善海洋)'),
        PrepItem(category='water', name='防水袋 (裝濕衣物)'),
        PrepItem(category='wear', name='換洗衣物 (短袖/短褲/薄外套)'),
        PrepItem(category='wear', name='拖鞋 / 涼鞋'),
        PrepItem(category='wear', name='防曬外套 / 袖套 / 帽子'),
        PrepItem(category='wear', name='太陽眼鏡'),
        PrepItem(category='other', name='手機充電器 / 行動電源'),
        PrepItem(category='other', name='個人藥品 (暈船藥/腸胃藥)'),
        PrepItem(category='other', name='防蚊液 (小黑蚊多)')
    ]
